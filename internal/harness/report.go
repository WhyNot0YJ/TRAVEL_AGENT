...
func printConsoleReport(w io.Writer, report Report) {
	...
	fmt.Fprintf(w, "Tool Fallback Rate: %.2f%%\n", s.ToolFallbackRate*100)
	fmt.Fprintf(w, "LLM Fallback Rate: %.2f%%\n", s.LLMFallbackRate*100)
	fmt.Fprintf(w, "External API Success Rate: %.2f%%\n", s.ExternalAPISuccessRate*100)
	fmt.Fprintf(w, "Average Node Duration: %.0fms\n", s.AverageNodeDurationMs)
	fmt.Fprintf(w, "Average Token Usage: %.0f\n", s.AverageTokenUsage)
	fmt.Fprintf(w, "Route Feasibility Pass Rate: %.2f%%\n", s.RouteFeasibilityPassRate*100)
	fmt.Fprintf(w, "Warning Rate: %.2f%%\n", s.WarningRate*100)
	fmt.Fprintln(w)
