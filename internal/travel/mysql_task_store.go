...
UPDATE travel_tasks
SET request_hash = ?, status = ?, request_json = ?, error_text = ?, updated_at = ?
WHERE id = ?`,
			task.RequestHash,
			task.Status,
			string(requestJSON),
...
SELECT t.id, t.request_hash, t.status, t.request_json, COALESCE(t.error_text, ''), t.created_at, t.updated_at, p.plan_json
FROM travel_tasks t
...
INSERT INTO travel_tasks (id, request_hash, status, request_json, error_text, created_at, updated_at)
VALUES (?, ?, ?, ?, ?, ?, ?)`,
		task.ID,
		task.RequestHash,
		task.Status,
