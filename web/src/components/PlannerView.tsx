...
import { createTravelPlanTask, savePlan, streamChatTravelInfo } from "../api/client";
import type { AgentMode, ChatRequest, TravelPlanRequest } from "../api/types";
import AgentConversation, { type ChatMessage, type TravelBrief } from "./AgentConversation";
import PlanDetail from "./PlanDetail";
import PlanProgress from "./PlanProgress";
import StateView from "./StateView";
import TravelBriefPanel from "./TravelBriefPanel";
...
function missingFields(brief: TravelBrief): string[] {
  ...
  if (!brief.departureCity) missing.push("出发城市");
  if (!brief.destinationCity) missing.push("目的地");
  if (!brief.days) missing.push("天数");
  if (!brief.budget) missing.push("预算");
  if (brief.interests.length === 0) missing.push("兴趣偏好");
  if (!brief.travelers) missing.push("出行人数");
  return missing;
}
...
const PENDING_SAVE_KEY = "ta_pending_save_task";
...
export default function PlannerView() {
  ...
  // After login, replay any pending save action.
  useEffect(() => {
    if (auth.status !== "authenticated" || !taskId || savedPlanId || !stream.plan) return;
    const pending = window.sessionStorage.getItem(PENDING_SAVE_KEY);
    if (pending && pending === taskId) {
      window.sessionStorage.removeItem(PENDING_SAVE_KEY);
      void persistPlan();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [auth.status, taskId, stream.plan, savedPlanId]);
  ...
  const acceptText = async (text: string) => {
    ...
    try {
      ...
      const response = await streamChatTravelInfo(payload, (chunk) => {
        setMessages((current) =>
          current.map((message) => (message.id === assistantId ? { ...message, text: `${message.text}${chunk}` } : message)),
        );
      });
  ...
  const handleGenerate = async (request: TravelPlanRequest) => {
    ...
    try {
      ...
      if (status === 401) {
        toast.show("请先登录后再生成路线");
        navigate(`/login?return_to=${encodeURIComponent("/planner")}`);
        return;
      }
  ...
  const persistPlan = async () => {
    if (!taskId || !stream.plan) return;
    setSaving(true);
    try {
      const saved = await savePlan({ task_id: taskId });
      setSavedPlanId(saved.plan_id);
      toast.show("已保存到我的计划");
    } catch (error) {
      const status = (error as { status?: number }).status;
      if (status === 401) {
        window.sessionStorage.setItem(PENDING_SAVE_KEY, taskId);
        toast.show("请先登录,登录后会继续保存");
        navigate(`/login?return_to=${encodeURIComponent(`/planner?task_id=${taskId}`)}`);
        return;
      }
      toast.show(error instanceof Error ? error.message : "保存失败,请稍后重试");
    } finally {
      setSaving(false);
    }
  };
  ...
  const planReady = Boolean(stream.plan);

  return (
    <div className="agent-shell" style={{ padding: 0, background: "transparent" }} data-testid="planner-view">
      {planReady ? (
        <div className="ta-card" style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
          <div>
            <strong style={{ display: "block", fontSize: 16 }}>{stream.plan?.title}</strong>
            <span style={{ color: "var(--ta-slate)", fontSize: 13 }}>生成完成,你可以保存到我的计划。</span>
          </div>
          {savedPlanId ? (
            <button
              type="button"
              className="ta-action-button secondary"
              onClick={() => navigate(`/me/plans/${savedPlanId}`)}
              data-testid="planner-view-saved"
            >
              查看我的计划
            </button>
          ) : (
